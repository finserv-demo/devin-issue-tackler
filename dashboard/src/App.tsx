...
function Tooltip({ text, children }: { text: string; children: React.ReactNode }) {
  ...
  return (
    ...
    >
      {children}
      <span
        className={`pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 -translate-x-1/2 whitespace-nowrap rounded-md bg-gray-900 px-2.5 py-1.5 text-xs text-white shadow-lg transition-opacity ${show ? 'opacity-100' : 'opacity-0'}`}
      >
        {text}
